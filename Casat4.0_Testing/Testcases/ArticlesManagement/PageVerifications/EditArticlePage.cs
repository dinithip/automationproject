...
namespace Casat4._0_Testing.Testcases.ArticlesManagement.PageVerifications
{
    ...
    public class EditArticlePage : BaseTest
    {
        ...
        public void TestMethod_editArticlePage()
        {
            ...
            Element editpgTitle = objarticle.editpagetitle;
            Assert.IsTrue(editpgTitle.InnerText.Contains(""));

            // Verify labelnames
            Element editnumlabl = objarticle.editnumberlabel;
            Assert.IsTrue(editnumlabl.InnerText.Contains(""));

            Element editnamelbl = objarticle.editnamelabel;
            Assert.IsTrue(editnamelbl.InnerText.Contains(""));

            Element editamountlbl = objarticle.editamountlabel;
            Assert.IsTrue(editamountlbl.InnerText.Contains(""));

            Element edittypelbl = objarticle.edittypelabel;
            Assert.IsTrue(edittypelbl.InnerText.Contains(""));
